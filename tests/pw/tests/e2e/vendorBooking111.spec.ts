...
test.describe('Booking Product test', () => {
    ...
    test.beforeAll(async ({ browser }) => {
        ...
        const vendorContext = await browser.newContext({
            ...data.auth.vendorAuth,
            logger: {
                isEnabled: (name, severity) => name === 'apii',
    ...
    test.only('vendor booking menu page is rendering properly', { tag: ['@pro', '@exp', '@vendor'] }, async () => {
        await vendor.vendorBookingtest();
    });
